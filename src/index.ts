...
class Index {
  ...
  main(args?: string[]) {
    const result = yargs(args)
        .options({
          ...
          example: {
            optional: true,
            describe: 'Run example input rather than real input for the given part/day.',
            alias: 'e'
          }
        })
        .version(false)
    ...
    const hasExample = Object.keys(result).includes("example");

    result.latest ? Index.runOne(Index.DAYS.length, part, hasExample)
        : result.day ? Index.runOne(result.day, part, hasExample)
    ...
    if (example) {
      return await Promise.all(
          puzzles.flatMap(async a => [
            {part: example, answer: await (example === 1 ? a.part1() : a.part2())}
          ])
      );
    }
  ...
  private static async runAll(hasExample?: boolean) {
    let solutions;
    if (hasExample) {
    ...
    // console.log(JSON.stringify({solutions}, undefined, 4))
  ...
  private static async runOne(dayNumber: number, part?: 1 | 2, hasExample?: boolean) {
    ...
    if (hasExample) {
      if (!part || part === 1) {
        const day = await new Index.DAYS[dayNumber - 1]().init(1);
        solution.push({part: 1, answer: await day.part1()})
      }
      if (!part || part === 2) {
        const day = await new Index.DAYS[dayNumber - 1]().init(2);
